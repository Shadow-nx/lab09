...
void Producer::producing2() {
    ...
    for(unsigned i = 0; i < counts; ++i){
        GumboOutput* output = gumbo_parse(fut[i].get().c_str());
...
void Producer::producing(std::string url, unsigned depth,Consumer& k){
    unsigned count = 0;
...
std::string Producer::parse_url_to_host(std::string url){
    if (url.find("https://") == 0)
        url = url.substr(8);
...
std::string Producer::parse_url_to_target(std::string url){
    if (url.find("https:") == 0)
        url = url.substr(8);
